...
class WCSimPrimaryGeneratorAction : public G4VUserPrimaryGeneratorAction {
  public:
    ...
    // Gun, laser & gps setting calls these functions to fill jhfNtuple and Root
    // tree
  ...
  private:
    ...
    // Temporary vertex that is saved if desired, according to WCSimIO macro
    // option
  ...
  public:
    ...
    inline void SetConversionProductParticle(int i, const G4ParticleDefinition *p) {
        conversionProductParticle[i] = p;
    }
    inline void SetConversionProductMomentum(int i, const G4ThreeVector &p) { conversionProductMomentum[i] = p; }
    inline void SetNeedConversion(bool choice) {
        needConversion = choice;
        foundConversion = !choice;
    }

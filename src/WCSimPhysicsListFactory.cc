...
WCSimPhysicsListFactory::WCSimPhysicsListFactory() : G4VModularPhysicsList() {
  ...
  PhysicsListName =
      "NULL_LIST"; // default list is set in WCSimPhysicsListFactoryMessenger to
                   // FTFP_BERT
  ...
  PhysicsMessenger =
      new WCSimPhysicsListFactoryMessenger(this, ValidListsString);
}
...
void WCSimPhysicsListFactory::ConstructProcess() {
  ...
  if (nCaptModelChoice.compareTo("Default", G4String::ignoreCase) == 0)
    return;
  G4ProcessTable *table = G4ProcessTable::GetProcessTable();
  G4ProcessManager *manager = G4Neutron::Neutron()->GetProcessManager();
  if (manager) {
    G4VProcess *captureProcess =
        table->FindProcess("nCapture", G4Neutron::Neutron());

    if (captureProcess) {
      G4cout << "Removing default nCapture process" << G4endl;
      manager->RemoveProcess(captureProcess);
      // Due to bug in Geant (fixed in v4.10.2) must remove old NeutronHPCapture
      // from registry, if it exists, to prevent segfault on cleanup
    ...
    G4NeutronCaptureXS *xsNeutronCaptureXS =
        (G4NeutronCaptureXS *)G4CrossSectionDataSetRegistry::Instance()
            ->GetCrossSectionDataSet(G4NeutronCaptureXS::Default_Name());
...
void WCSimPhysicsListFactory::SetList(G4String newvalue) {
  G4cout << "Setting Physics list to " << newvalue
...
void WCSimPhysicsListFactory::InitializeList() {
  ...
  if (factory->IsReferencePhysList(PhysicsListName)) {
    ...
    G4cout << "Physics list " << PhysicsListName << " is not understood"
           << G4endl;
  }
}

...
WCSimPrimaryGeneratorMessenger::WCSimPrimaryGeneratorMessenger(WCSimPrimaryGeneratorAction *pointerToAction)
    ...
    genCmd->SetGuidance(" Available generators : muline, gun, laser, gps, "
                        "ibd, datatable, cosmics, radioactive, "
                        "rootracker, radon, injector, gamma-conversion");
    genCmd->SetParameterName("generator", true);
    genCmd->SetDefaultValue("muline");
    genCmd->SetCandidates("muline gun laser gps ibd datatable cosmics radioactive rootracker "
                          "radon injector, gamma-conversion");
    ...
    // C. Vilela: Adding PMTPoisson for generating photoelectrons directly on PMTs
    // according to a Poisson distribution.
    poisCmd = new G4UIcmdWithABool("/mygen/pmtPoisson", this);
    poisCmd->SetGuidance("Flag for generating photoelectrons directly on PMTs according to a "
                         "Poisson distribution. These PE's will be generated in addition to light "
                         "produce by any particles generated. Set dark rate to 0 and do not "
                         "generate any particles for events with only Poisson PE's.");
    ...
    timeUnitCmd->SetGuidance("Can be picosecond, ps, ns, nanosecond, ms, "
                             "millisecond, s, sec or second");
    ...
    radonScalingCmd->SetGuidance("Select scalling scenario, if scenario 0 is "
                                 "selected, Bi214 are generated uniformly");
...
void WCSimPrimaryGeneratorMessenger::SetNewValue(G4UIcommand *command, G4String newValue) {
    if (command == genCmd) {
        ...
        } else if (newValue == "injector") // addition of injector events
        {
            ...
            myAction->SetRadioactiveEvtGenerator(false);
            myAction->SetRadonEvtGenerator(false);
        } else if (newValue == "radioactive") // G. Pronost: Addition of
                                              // Radioactivity (from F. Nova code)
        {
            myAction->SetMulineEvtGenerator(false);
            myAction->SetGunEvtGenerator(false);
            myAction->SetRootrackerEvtGenerator(false);
            ...
            myAction->SetRadonEvtGenerator(false);
        } else if (newValue == "radon") // G. Pronost: Addition of Radon generator (based on F.
                                        // Nova's radioactive generator but dedicated to
                                        // radioactive events in water)
        {
            myAction->SetMulineEvtGenerator(false);
            myAction->SetGunEvtGenerator(false);
            myAction->SetRootrackerEvtGenerator(false);
    ...
    if (command == fileNameCmd) {
        if (genSet) {
            ...
            G4cout << "Generator has not been set, guessing input vector file is NOT "
                      "in the Rootracker format - this will crash if you are using a "
                      "Rootracker input file"
                   << G4endl;
            G4cout << "Please put the '/mygen/generator' command above the "
                      "'/mygen/vecfile' command in the mac file."
                   << G4endl;
        }
    }
    ...
    if (command == poisCmd) {
        if (poisCmd->GetNewBoolValue(newValue)) {
            ...
            G4cout << "Running with PoissonPMT flag. Photoelectrons will be "
                      "generated directly on the PMTs according to a Poisson "
                      "distribuition. Any hits resulting from physics generated "
                      "elsewhere will be discarded !!!"
                   << G4endl;
        } else
            myAction->SetPoissonPMT(false);
    }
...
G4String WCSimPrimaryGeneratorMessenger::GetCurrentValue(G4UIcommand *command) {
    ...
    if (command == genCmd) {
        if (myAction->IsUsingMulineEvtGenerator()) {
            ...
            cv = "rootracker";
        } // M. Scott: Addition of Rootracker events
...
void WCSimPrimaryGeneratorMessenger::RadonScalingCommand(G4String newValue) {
    ...
    if (scenario == "A")
        iScenario = 1; // Relative scaling with respect to full ID volume (Pessimistic)
    if (scenario == "B")
        iScenario = 2; // Relative scaling with respect to fiducial volume

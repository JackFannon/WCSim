...
WCSimPrimaryGeneratorAction::~WCSimPrimaryGeneratorAction() {
    if (IsGeneratingVertexInRock()) {
        ...
        G4cout << " Random number generated in Rock / in Cublic = " << _counterRock << "/" << _counterCublic
    ...
    if (useMulineEvt) {
        ...
        if (useNuanceTextFormat) {
            ...
            if (token.size() == 0 || token[0] == "stop") {
                ...
                G4cout << "unexpected line begins with \"" << token[0] << "\"we were expecting \" begin \""
                       << G4endl;
                ...
                while (token[0] == "nuance" && iVertex < MAX_N_VERTICES) {
                    ...
                    token = readInLine(inputFile, lineSize, inBuf);
                    vtxs[iVertex] = G4ThreeVector(atof(token[1]) * cm, atof(token[2]) * cm, atof(token[3]) * cm);
                    ...
                    beamdirs[iVertex] = G4ThreeVector(atof(token[3]), atof(token[4]), atof(token[5]));
                    ...
                    while (token = readInLine(inputFile, lineSize, inBuf), token[0] == "track") {
                        ...
                        targetdirs[iVertex] = G4ThreeVector(atof(token[3]), atof(token[4]), atof(token[5]));
                        G4cout << "Target hit is = " << targetpdgs[iVertex] << ", E = " << targetenergies[iVertex]
                               << G4endl;
...
#pragma GCC diagnostic pop
            ...
            G4double random_z = ((myDetector->GetWaterTubePosition()) - .5 * (myDetector->GetWaterTubeLength()) +
                                 1. * m + 15.0 * m * G4UniformRand()) /
                                m;
            zPos = random_z;
        ...
        double kin_energy = momentumGeV; // fabs(fTmpRootrackerVtx->StdHepP4[i][3])*GeV -
        ...
        for (int i = 0; i < fTmpRootrackerVtx->StdHepN; i++) {
            ...
            kin_energy =
                fabs(fTmpRootrackerVtx->StdHepP4[i][3]) * GeV - particleGun->GetParticleDefinition()->GetPDGMass();
        ...
        G4cout << "Name: " << particleGun->GetParticleDefinition()->GetParticleName()
...
#pragma GCC diagnostic pop
        ...
        G4ThreeVector P = anEvent->GetPrimaryVertex()->GetPrimary()->GetMomentum();
        G4ThreeVector vtx = anEvent->GetPrimaryVertex()->GetPosition();
        G4double mass = anEvent->GetPrimaryVertex()->GetPrimary()->GetMass(); // will be 0 for photon anyway, but
                                                                              // for other gps particles not
        G4int pdg = anEvent->GetPrimaryVertex()->GetPrimary()->GetPDGcode();
    ...
    } else if (useInjectorEvt) // K.M.Tsui: addition of injector events
    {
        ...
        for (int i = 0; i < nLayerInjectors; i++) {
            double zpos = myDetector->GetWCIDHeight() / (nLayerInjectors + 1.) * (i + 1.) -
                          myDetector->GetWCIDHeight() / 2.; // evenly spaced in z
            for (int j = 0; j < nInjectorsPerLayer; j++) {
                double dist_to_center =
                    myDetector->GetWCIDDiameter() / 2. - 20 * cm; // not sure the exact value so just a guess
        ...
        for (int i = 0; i < 2; i++) {                                 // top and bottom endcap
            ...
            for (int j = 0; j < nCapInjectors; j++) {
                double dist_to_center =
                    0.12 * myDetector->GetWCIDDiameter() / 2.; // again the exact value is unknown
                double theta_start = 0; // may want some offset to avoid overlap with other components
                double theta_current = theta_start + j * CLHEP::pi / 2.;
                double xpos = dist_to_center * cos(theta_current);
                double ypos = dist_to_center * sin(theta_current);
        ...
        G4ThreeVector dir = P.unit();
        G4double E = std::sqrt((P.dot(P)) + (mass * mass));
        G4cout << " GPS primary vertex (" << vtx.x() << ", " << vtx.y() << ", " << vtx.z() << "), dir (" << dir.x()
               << ", " << dir.y() << ", " << dir.z() << ") m " << m << " E " << E << " pdg " << pdg << G4endl;
        ...
        if (needConversion) {
            ...
            primaryParticle->SetMomentum(conversionProductMomentum[0].getX(), conversionProductMomentum[0].getY(),
                                         conversionProductMomentum[0].getZ());
            G4PrimaryParticle *secondProduct =
                new G4PrimaryParticle(conversionProductParticle[1], conversionProductMomentum[1].getX(),
                                      conversionProductMomentum[1].getY(), conversionProductMomentum[1].getZ());
        ...
        */
        ...
        // DEBUG PRINTS
        G4cout << G4endl;
        G4cout << "Generated at position : " << vtx.getX() / m << "m " << vtx.getY() / m << "m " << vtx.getZ() / m
               << "m " << G4endl;
        ...
        for (it = radioactive_sources.begin(); it != radioactive_sources.end(); it++) {
            ...
            for (int u = 0; u < n_vertices; u++) {
                ...
                if (IsotopeLocation.compareTo("water") == 0) {
                    ...
                    MyGPS->GetCurrentSource()->GetPosDist()->SetRadius(myDetector->GetGeo_Dm(3) * CLHEP::cm -
                                                                       2. * PMT->GetRadius());
                    MyGPS->GetCurrentSource()->GetPosDist()->SetHalfZ(myDetector->GetGeo_Dm(2) * CLHEP::cm / 2. -
                                                                      2. * PMT->GetRadius());
                    ...
                    G4ThreeVector position =
                        random_pmt_center +
                        random_R * (orientation * random_cos_theta + axis_1 * random_sin_theta * cos(random_phi) +
